...
class DecoderOnPairing(BaseModel):
    ...
    def forward(image,gtBBs,gtTrans,question,answer=None):
        ...
        Maybe layoutlm as well, to account for times its been processed in two batches

        memory_feats = torch.cat((layoutlm_feats,graph_feats,question_feats),dim=2) #place the graph_feats between so it gets put between the SEP and START tokens
