...
class PairingGroupingGraph(BaseModel):
            ...
            if self.useMetaGraph:
                ...
                for gIter,graphnet in enumerate(self.graphnets[1:]):
                    ...
                    if self.reintroduce_visual_features:
                        graph,last_node_visual_feats,last_edge_visual_feats = self.appendVisualFeatures(useBBs,graph,groups)
    ...
    def appendVisualFeatures(self,useBBs,graph,groups,same_node_map,prev_node_visual_feats,prev_edge_visual_feats,prev_edge_indexes):
        ...
        node_visual_feats = torch.FloatTensor(node_features.size(0),self.prev_node_visual_feats.size(1))
        has_feat = [False]*node_features.size(0)
        for old_id,new_id in same_node_map:
