...
def resize(img,dim=None,fx=None,fy=None,order=3): #remove ",interpolation = cv2.INTER_CUBIC"
    ...
    if dim is None or dim[0]==0:
        ...
        try:
            ...
            return transform.resize(img,[h,w],order,anti_aliasing=downsize,preserve_range=True)
        ...
        return transform.resize(img,dim,order,anti_aliasing=downsize,preserve_range=True)
...
def remap(img,map_x,map_y,interpolation=2,borderValue=0):
