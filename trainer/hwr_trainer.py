...
class HWRTrainer(BaseTrainer):
    ...
    def __init__(self, model, loss, metrics, resume, config,
        ...
        super(HWRTrainer, self).__init__(model, loss, metrics, resume, config, train_logger)
        self.config = config
    ...
    def _train_iteration(self, iteration):
        ...
        pred, pred_str, losses = self.run(instance)
    ...
    def _valid_epoch(self):
        ...
        with torch.no_grad():
            ...
            for batch_idx, instance in enumerate(self.valid_data_loader):
                ...
                if pred_str is not None:
                    gt = instance['gt']
                    cer, cer_ind = self.getCER(gt,pred_str,True)
                    total_cer += cer
    ...
    def getCER(self,gt,pred_strs,individual=False):
        cer=0
        if individual:
            all_cer=[]
        for i,gt_line in enumerate(gt):
