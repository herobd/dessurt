...
def FUNSDGraphPair_eval(config,instance, trainer, metrics, outDir=None, startIndex=None, lossFunc=None, toEval=None):
    ...
    if useDetections=='gt':
        ...
        losses, log, out = trainer.run(instance,False,get=toEval)

    if trackAtt:
        attList = model.pairer.attn
    #relPredFull = relPred
    relPred = out['relPred']
    relIndexes = out['relIndexes']
    bbPred = out['bbPred']
    outputBoxes = out['outputBoxes']
                    ...
                    if model.predNN:
                        ...
                        if toKeep.any():
                            ...
                            if target_num_neighbors is not None:
                                target_num_neighbors_use = torch.cat((target_num_neighbors[0].float(),torch.zeros(1)),dim=0)
                            else:
                    ...
                    if model.predClass:
                        ...
                        if targetBoxes is not None:
                            ...
                            if toKeep.any():
                                ...
                                bbAlignment_use = bbAlignment[toKeep]
                                alignedClass_use =  targetBoxes[0][bbAlignment_use][:,13:13+model.numBBTypes] #There should be no -1 indexes in hereS
                            else:
    ...
    if trackAtt:
        ...
        for j in range(bbs.shape[0]):
            ...
            if outDir is not None:
                ...
                maxIndex =np.argmax(bbs[j,6+extraPreds:10+extraPreds])
                if maxIndex==0:
