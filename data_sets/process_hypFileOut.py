...
def addToJSON(directory,image,lines):
    ...
    if isinstance(data,list):
        ...
        data['recognition'] = lines
    with open(path,'w') as f:
        json.dump(data,f,indent=1)
    print('wrote {}'.format(path))

def process(hypFileOut,json_dir):
    remove='⌨┃'

    last_image=None
    #check='005449024_00310.jpg-A-00200'

    with open(hypFileOut) as f:
        textlines = f.readlines()

    for ind,textline in enumerate(textlines):
        ...
        if image!=last_image:
            if last_image is not None:
                print('finished {} ({}/{})'.format(last_image,ind,len(textlines)))
                #resolveOverlap(lines,bbs)
                #img_f.imshow('x',draw_img)
                #img_f.show()
                addToJSON(json_dir,last_image,lines)
            ...
            if len(word)>0:
                ...
                if full_line is None:
                    ...
                    if x1<x2 and y1<y2:
                        ...
                        break #for Census, never clockwise
    ...
    print('finished '+last_image)
    addToJSON(json_dir,last_image,lines)

hyp = 'all_states_random_4k_KEYOUTS/hypFileOut.all_states_random_4k_Mx8M.BBOX'
json_dir = '1940_all-states_random_4k_index_cleaned'
process(hyp,json_dir)
